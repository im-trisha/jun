...
impl Screens {
    pub fn title(&self, language: Language) -> String {
        let s = match self {
            ...
            Self::GameProgression(_) => language.t_screens_game_progression_title(),
        };
        s.into()
    }
    ...
    pub fn show(&mut self, ui: &mut egui::Ui, state: &mut JunAppState) {
        match &mut *self {
            Self::Empty(empty_screen) => {
                empty_screen.ui(ui, state);
            }
            Self::SaveSlotPicker(save_slot_picker) => {
                save_slot_picker.ui(ui, state);
            }
            Self::JunStats(jun_stats) => {
            ...
            Self::GameProgression(game_progression) => {
